	...
	protected void paintComponent(Graphics g) {
        ...
        for(int y = 0; y < data.getHeight(); y++){
            ...
            for(int x = 0; x < data.getWidth(); x++){
            	drawHoriBlock(g, x * length + padding, y * length + padding);
            }
            //Vertical Lines
            for(int x = data.getWidth(); x < data.getWidth() * 2 + 1; x++){
                drawVertBlock(g, x * length - length + padding, y * thickness + padding);
            }
        }
        //gets bottom line
        for(int x = 0; x < data.getWidth(); x++){
        	drawHoriBlock(g, x * length + padding, data.getHeight() * length);
        }
	}
	
	private void drawVertBlock(Graphics g, int x, int y){
		g.drawRect(x, y, thickness, length + thickness);
		g.fillRect(x, y, thickness, length + thickness);
	}
	
	private void drawHoriBlock(Graphics g, int x, int y){
